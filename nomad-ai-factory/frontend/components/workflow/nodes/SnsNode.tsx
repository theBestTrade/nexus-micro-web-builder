...
const SnsNode: React.FC<{ data: any }> = ({ data }) => {
  const getStatusStyles = () => {
    switch (data.status) {
      case 'running': return 'border-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.5)] animate-pulse';
      ...
      default: return 'border-border hover:border-indigo-500/50';
    }
  };
  ...
  return (
    <div className={`px-4 py-3 shadow-lg rounded-xl bg-card border-2 flex items-center gap-3 min-w-[220px] transition-all ${getStatusStyles()}`}>
      ...
      <div className="p-2 rounded-lg bg-indigo-500/10 text-indigo-500 relative">
        <Share2 size={20} />
        {data.status === 'running' && <Loader2 size={12} className="absolute -top-1 -right-1 animate-spin text-indigo-500 bg-background rounded-full" />}
      ...
      <div className="flex-1">
        <div className="text-sm font-bold text-foreground">{data.label || 'SNS 자동 발행'}</div>
        <div className="text-xs text-muted-foreground">{data.platform || '소셜 미디어'}</div>
      </div>

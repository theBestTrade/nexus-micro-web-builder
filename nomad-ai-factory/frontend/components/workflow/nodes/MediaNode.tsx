...
const MediaNode: React.FC<{ data: any }> = ({ data }) => {
  ...
  return (
    <div className={`px-4 py-3 shadow-lg rounded-xl bg-card border-2 flex items-center gap-3 min-w-[220px] transition-all ${getStatusStyles()}`}>
      ...
      <div className="p-2 rounded-lg bg-pink-500/10 text-pink-500 relative">
        <Scissors size={20} />
      ...
      <div className="flex-1">
        <div className="text-sm font-bold text-foreground">{data.label || '영상 편집 & TTS'}</div>
        <div className="text-xs text-muted-foreground">FFmpeg 렌더링</div>
      </div>

...
const SourceNode: React.FC<{ data: any }> = ({ data }) => {
  ...
  return (
    <div className={`px-4 py-3 shadow-lg rounded-xl bg-card border-2 flex items-center gap-3 min-w-[220px] transition-all ${getStatusStyles()}`}>
      <div className="p-2 rounded-lg bg-blue-500/10 text-blue-500 relative">
        <Video size={20} />
      ...
      <div className="flex-1">
        <div className="text-sm font-bold text-foreground">{data.label || '소스 입력'}</div>
        <div className="text-xs text-muted-foreground">원본 영상 / URL</div>
      </div>

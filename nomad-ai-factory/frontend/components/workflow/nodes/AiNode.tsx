...
const AiNode: React.FC<{ data: any }> = ({ data }) => {
  ...
  return (
    <div className={`px-4 py-3 shadow-lg rounded-xl bg-card border-2 flex items-center gap-3 min-w-[220px] transition-all ${getStatusStyles()}`}>
      ...
      <div className="p-2 rounded-lg bg-purple-500/10 text-purple-500 relative">
        <Languages size={20} />
      ...
      <div className="flex-1">
        <div className="text-sm font-bold text-foreground">{data.label || 'AI 번역 및 대본'}</div>
        <div className="text-xs text-muted-foreground">Gemini 1.5 Pro</div>
      </div>
